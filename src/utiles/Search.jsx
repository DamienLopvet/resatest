...
export default function SearchBar({ events }) {
    ...
    return (
        <div className="absolute hidden h-full w-screen" id="searchBar_container">
            <div id="searchBar_curtain" className="absolute top-0 left-0 right-0 bottom-0 z-40 bg-slate-400 opacity-30" ></div>
            <div className="relative shadow-md rounded-md min-h-[400px] z-40 flex justify-start p-5 bg-white top-20 left-2/4 w-2/4 -translate-x-2/4 ">
                <div className="flex justify-start items-start align-bottom  border-b border-slate-300 w-full h-10">
                    ...
                    <input type="text" className="relative outline-none" id="search_input" autoComplete="off" placeholder="Recherche rapide" />
                </div>
            </div>
        </div>
    );
